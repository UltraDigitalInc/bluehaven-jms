$(document).ready(function()
{
   ...
   $('#TrashBinPurge').live('click',function() {
      //alert('Test');
      var x = $(this).position().left + $(this).outerWidth();
      var y = $(this).position().top - $(document).scrollTop();
      
      $('#TrashBinPurgeDialog').dialog('option', 'position', [x,y]);
      $('#TrashBinPurgeDialog').dialog('open');
   });
      ...
      buttons: {
         Upload: function() {
            if ($('#nfuserfile').val()!='')
            {
               ...
               /*
               var poid    = $('#FFOID').val();
               var pfldid  = $('#ListingParentId').val();
               var pnfuid  = $('#nfuid').val();
               var pnfstor = $('#nfstoretype').val();
               var pnfuf   = $('#nfuserfile').val();
               var pnfmfs  = $('#nfMAX_FILE_SIZE').val();
               //alert('Settings: ' + poid + '.' + pfldid + '.' + pnfuid + '.' + pnfstor + '.' + pnfuf + '.' + pnfmfs)
               
               if (poid && pfldid && pnfuid && pnfstor && pnfuf && pnfmfs)
               {
                  $.ajax({
                     cache:false,
                     type : 'POST',
   ...
   $('.FileAdd').live('click',function() {
      ...
      $('#DivAddFileFormElement').append('<input type="hidden" name="fscid" id="ParentId" value="' + fidpid + '">');
      $('#nfuserfile').val('').focus();
