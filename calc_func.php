...
function calc_royalty_est($costitems)
{
	...
	*/
	...
	{
		$preval=100000;
		if (($viewarray['camt']-$viewarray['royrel']) >= $preval)
		{
			
			$preroy = ($preval *.03);
			$pstroy = ((($viewarray['camt']-$viewarray['royrel']) - $preval ) * .01);
			$royalty= $preroy + $pstval;
		}
		else
		{
		...
		foreach ($_REQUEST as $n=>$v)
		{
			if (substr($n,0,4)=="bbba")
			{
				...
				if ($_REQUEST['fffa'.$asid]!=32)
				{
					if ($_REQUEST['bbba'.$asid] > 0)
					{
						if (array_key_exists("aaaa".$asid,$_REQUEST))
						{
							//if (array_key_exists("ccca".$asid,$_REQUEST))
							//{
								if (array_key_exists("ddda".$asid,$_REQUEST))
								{
									if (array_key_exists("ggga".$asid,$_REQUEST))
									{
										if (array_key_exists("hhha".$asid,$_REQUEST))
										{
											if (array_key_exists("iiia".$asid,$_REQUEST))
											{
												if (array_key_exists("code".$asid,$_REQUEST))
												{
													$code=$_REQUEST['code'.$asid];
												}
												else
												{
													$code=0;
												...
												if ($icount==1)
												{
													$estd=$_REQUEST['aaaa'.$asid].':0:'.$_REQUEST['bbba'.$asid].':'.$_REQUEST['ddda'.$asid].':'.$code.':'.$_REQUEST['ggga'.$asid].':'.$_REQUEST['hhha'.$asid].':'.$_REQUEST['iiia'.$asid];
												}
												else
												{
													$estd=$_REQUEST['aaaa'.$asid].':0:'.$_REQUEST['bbba'.$asid].':'.$_REQUEST['ddda'.$asid].':'.$code.':'.$_REQUEST['ggga'.$asid].':'.$_REQUEST['hhha'.$asid].':'.$_REQUEST['iiia'.$asid].',';
...
function edit_bid()
{
	...
	echo "								<td align=\"right\" valign=\"bottom\"><b>".$bidinfo."</b></td>\n";
	echo "							</tr>\n";
	echo "						</table>\n";
	echo "					</td>\n";
	echo "				</tr>\n";
...
function edit_mpa_jobmode_add()
{
	...
	$qry0 = "SELECT cid,estid,jobid,njobid FROM cinfo WHERE officeid='".$_SESSION['officeid']."' and ".$jfield."='".$trid."';";
	$res0 = mssql_query($qry0);
	$row0 = mssql_fetch_array($res0);
	...
	if ($nrow1 == 0)
	{
		if ($_SESSION['action']=="est")
		{
			...
			$qry .= "VALUES ('".$_SESSION['officeid']."',";
			$qry .= "'".$estid."','".$row0['jobid']."','".$fjadd."','".$_POST['phsid']."','".$_POST['rdbid']."',";
			$qry .= "'".$_POST['cdbid']."','".replacequote($_POST['partno'])."',";
			$qry .= "'".replacequote($_POST['vendor'])."','".replacequote($_POST['sdesc'])."',";
		...
		elseif ($_SESSION['action']=="job")
		{
			...
			$qry .= "VALUES ('".$_SESSION['officeid']."',";
			$qry .= "'".$estid."','".$row0['jobid']."','".$row0['njobid']."','".$fjadd."','".$_POST['phsid']."','".$_POST['rdbid']."',";
			$qry .= "'".$_POST['cdbid']."','".replacequote($_POST['partno'])."',";
			$qry .= "'".replacequote($_POST['vendor'])."','".replacequote($_POST['sdesc'])."',";
			$qry .= "'".replacequote($_POST['comments'])."','".number_format($_POST['bprice'], 2, '.', '')."','".$_POST['uid']."');";
			$res = mssql_query($qry);
...
function edit_bid_jobmode_add()
{
	...
	if ($nrow1 == 0)
	{
		if ($_SESSION['action']=="est")
		{
			...
			$qry .= "VALUES ('".$_SESSION['officeid']."',";
			$qry .= "'".$estid."','".$row0['jobid']."','0','".$_POST['phsid']."','".$_POST['rdbid']."',";
			$qry .= "'".$_POST['cdbid']."','".replacequote($_POST['partno'])."',";
			$qry .= "'".replacequote($_POST['vendor'])."','".replacequote($_POST['sdesc'])."',";
			$qry .= "'".replacequote($_POST['comments'])."','".number_format($_POST['bprice'], 2, '.', '')."','".$_POST['uid']."');";
			$res  = mssql_query($qry);
		...
		elseif ($_SESSION['action']=="contract")
		{
			...
			$qry .= "VALUES ('".$_SESSION['officeid']."',";
			$qry .= "'".$estid."','".$row0['jobid']."','".$_POST['jadd']."','".$_POST['phsid']."','".$_POST['rdbid']."',";
			$qry .= "'".$_POST['cdbid']."','".replacequote($_POST['partno'])."',";
			$qry .= "'".replacequote($_POST['vendor'])."','".replacequote($_POST['sdesc'])."',";
			$qry .= "'".replacequote($_POST['comments'])."','".number_format($_POST['bprice'], 2, '.', '')."','".$_POST['uid']."');";
			$res = mssql_query($qry);
		...
		elseif ($_SESSION['action']=="job")
		{
			...
			$qry .= "VALUES ('".$_SESSION['officeid']."',";
			$qry .= "'".$estid."','".$row0['jobid']."','".$row0['njobid']."','".$_POST['jadd']."','".$_POST['phsid']."','".$_POST['rdbid']."',";
			$qry .= "'".$_POST['cdbid']."','".replacequote($_POST['partno'])."',";
			$qry .= "'".replacequote($_POST['vendor'])."','".replacequote($_POST['sdesc'])."',";
			$qry .= "'".replacequote($_POST['comments'])."','".number_format($_POST['bprice'], 2, '.', '')."','".$_POST['uid']."');";
			$res = mssql_query($qry);
...
{	
	...
	if (isset($_SESSION['viewarray']) and count($_SESSION['viewarray']) > 0)
	{
		$viewarray	=$_SESSION['viewarray'];
	}
	//$viewarray	=$_SESSION['viewarray'];
	...
	*/
	...
	{
		$quan_out=$viewarray['camt'] * $def_quan;
		$subbp=$quan_out;
		$subrp=$quan_out;
	}
	else // Catch Bucket
	...
	$ar_out=array(0=>round($subbp),1=>$subrp,2=>$quan_out);
		...
		if ($_SESSION['securityid']==26)
		{
			...
			echo "*BASEO: ".$bp."<BR>";
			echo "*RETAO: ".$rp."<BR>";
			echo "*QUANT: ".$quan_out."<BR>";
			echo "*DQUAN: ".$def_quan."<BR>";
			echo "*HIGHR: ".$hr."<BR>";
			echo "*SBOUT: ".$subbp."<BR>";
			echo "*CHGPR: ".$chgproc."<BR>";
			echo "------------------------<BR>";
		}
		*/
...
function labor_baseitems_job_calc($phsid,$bdata,$jtag)
{
	...
	if (isset($viewarray['discount']) and $viewarray['discount']!=0)
	{
		$discount   =$viewarray['discount'];
	}
	else
...
function calc_adjusts()
{
	...
	$col_struct=array('125px','505px','30px','30px','30px','60px','60px','60px');
	...
	if ($nrowA > 0)
	{
		while ($rowA  = mssql_fetch_array($resA))
		{
			...
			echo "           <tr>\n";
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"left\"><b>Discount</b></td>\n";
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"left\"><img src=\"images/pixel.gif\"></td>\n";
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"left\"><img src=\"images/pixel.gif\"></td>\n";
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"left\">".$desc."</td>\n";
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"right\">";
			...
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"center\"><img src=\"images/pixel.gif\"></td>\n";
			echo "           </tr>\n";
			$tadj=$tadj+$adj;
		}
	...
	if ($_SESSION['action']=="est")
	{
		...
		//echo "			<table background=\"white\" bordercolor=\"gray\" width=\"875px\" border=1>\n";
		echo "			<tr>\n";
		echo "              <td class=\"wh\" align=\"right\" width=\"".$col_struct[0]."\">\n";
		echo "					<div class=\"noPrint\">\n";
		echo "						<b>New Discount</b>\n";
		echo "					</div>\n";
		echo "				</td>";
		echo "              <td class=\"wh\" align=\"left\" width=\"".$col_struct[1]."\">\n";
		echo "					<div class=\"noPrint\">\n";
		echo "						<input class=\"bboxl\" name=\"descrip\" type=\"text\" size=\"70\" maxlength=\"96\">\n";
		echo "					</div>\n";
		echo "				</td>\n";
		echo "              <td class=\"wh\" align=\"right\" width=\"".$col_struct[2]."\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\" width=\"".$col_struct[3]."\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\" width=\"".$col_struct[4]."\">\n";
		echo "					<div class=\"noPrint\">\n";
		echo "						<input class=\"bboxl\" name=\"adjamt\" type=\"text\" size=\"6\" maxlength=\"9\">\n";
		echo "					</div>\n";
		echo "				</td>";
		...
		if ($_SESSION['action']=="est")
		{
			...
			{
				echo "                  <input class=\"transnb\" type=\"image\" src=\"images/save.gif\" value=\"Apply\" title=\"Save Discount\" DISABLED>\n";
			}
			else
			{
				echo "                  <input class=\"transnb\" type=\"image\" src=\"images/save.gif\" value=\"Apply\" title=\"Save Discount\">\n";
			}
		}
		...
		echo "           <tr>\n";
		echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\"><b>Total Discount</b></td>\n";
		echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
...
function calc_adjusts_EXIST($commrate)
{
	global $discount,$viewarray;
	...
	//$col_struct=array('140px','560px','30px','30px','30px','60px','30px');

	if ($_SESSION['action']=="est")
	{
		$qrypre = "SELECT estid,jobid,njobid,mas_prep FROM cinfo WHERE officeid='".$_SESSION['officeid']."' AND estid='".$_SESSION['estid']."';";
		$respre = mssql_query($qrypre);
		$rowpre = mssql_fetch_array($respre);
	...
	elseif ($_SESSION['action']=="contract")
	{
		$qrypre = "SELECT estid,jobid,njobid,mas_prep FROM cinfo WHERE officeid='".$_SESSION['officeid']."' AND jobid='".$_POST['jobid']."';";
		$respre = mssql_query($qrypre);
		$rowpre = mssql_fetch_array($respre);
	...
	if ($nrowA > 0)
	{
		...
		while ($rowA  = mssql_fetch_array($resA))
		{
			...
			$fadj =number_format($adj, 2, '.', '');
			$fcomm =number_format(($adj * $commrate), 2, '.', '');
			
			echo "           <tr>\n";
			echo "              <td class=\"wh\" align=\"right\"><b>Price Adjustment</b></td>\n";
			echo "              <td class=\"wh\" align=\"left\">".$desc."</td>\n";
			echo "              <td class=\"wh\" align=\"left\"><img src=\"images/pixel.gif\"></td>\n";
			echo "              <td class=\"wh\" align=\"left\"><img src=\"images/pixel.gif\"></td>\n";
			echo "              <td class=\"wh\" align=\"right\">\n";
			...
			if (($adj * $commrate) < 0)
			{
				echo "<font color=\"red\">$fcomm</font>";
			}
			else
			{
				echo $fcomm;
			...
			echo "</td>\n";
			echo "              <td class=\"wh\" valign=\"bottom\" align=\"center\">\n";
			...
			if ($_SESSION['action']!="job")
			{
				...
				echo "					<form method=\"post\">\n";
				echo "					<input type=\"hidden\" name=\"action\" value=\"est\">\n";
				echo "					<input type=\"hidden\" name=\"call\" value=\"remove_est_adj_item\">\n";
				echo "					<input type=\"hidden\" name=\"officeid\" value=\"".$_SESSION['officeid']."\">\n";
				echo "					<input type=\"hidden\" name=\"estid\" value=\"".$_SESSION['estid']."\">\n";
				echo "					<input type=\"hidden\" name=\"iid\" value=\"".$rowA['id']."\">\n";
				echo "					<input type=\"hidden\" name=\"esttype\" value=\"E\">\n";
				echo "					<input class=\"transnb_button\" type=\"image\" src=\"images/delete.png\" title=\"Delete Adjust Item\">\n";
				echo "					</form>\n";
				echo "					</div>\n";
			}
			...
			echo "           </tr>\n";
			$tadj=$tadj+$adj;
			$tcom=$tcom+($adj * $commrate);
		}
		$ftadj =number_format($tadj, 2, '.', '');
		$ftcom =number_format($tcom, 2, '.', '');
		...
		echo "           <tr>\n";
		echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\"><b>Total Price Adjusts</b></td>\n";
		echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td class=\"wh\" align=\"right\">\n";
		...
		if ($tcom < 0)
		{
			echo "<font color=\"red\">".$ftcom."</font>";
		}
		else
		{
			echo $ftcom;
		...
		$discount=$tadj;
		$adj_out=array($discount,$tcom);
	}
	
	return $adj_out;
}

function calc_adjusts_NEW($col_struct)
{
	if ($_SESSION['action']=="est")
	{
		echo "			<tr>\n";
		echo "              <td align=\"right\" width=\"".$col_struct[0]."\">\n";
		echo "					<div class=\"noPrint\">\n";
		echo "						<b>New Discount</b>\n";
		echo "					</div>\n";
		echo "				</td>";
		echo "              <td align=\"left\" width=\"".$col_struct[1]."\">\n";
		echo "					<div class=\"noPrint\">\n";
		echo "						<form method=\"post\">\n";
		echo "							<input type=\"hidden\" name=\"action\" value=\"est\">\n";
		echo "							<input type=\"hidden\" name=\"call\" value=\"adjins\">\n";
		...
		echo "							<input type=\"hidden\" name=\"officeid\" value=\"".$_SESSION['officeid']."\">\n";
		echo "							<input type=\"hidden\" name=\"securityid\" value=\"".$_SESSION['securityid']."\">\n";
		echo "							<input class=\"bboxl\" name=\"descrip\" type=\"text\" size=\"60\" maxlength=\"96\">\n";
		echo "					</div>\n";
		echo "				</td>\n";
		echo "              <td align=\"right\" width=\"".$col_struct[2]."\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td align=\"right\" width=\"".$col_struct[3]."\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td align=\"right\" width=\"".$col_struct[4]."\">\n";
		echo "					<div class=\"noPrint\">\n";
		echo "							<input class=\"bboxl\" name=\"adjamt\" type=\"text\" size=\"6\" maxlength=\"9\">\n";
		echo "					</div>\n";
		echo "				</td>";
		echo "              <td align=\"right\" width=\"".$col_struct[5]."\"><img src=\"images/pixel.gif\"></td>\n";
		echo "              <td align=\"center\" width=\"".$col_struct[6]."\">\n";
		echo "					<div class=\"noPrint\">\n";
			
		if ($_SESSION['action']=="est")
		{
			if ((isset($rowpre['jobid']) and $rowpre['jobid']!=0) and (isset($rowpre['njobid']) and $rowpre['njobid']!=0))
			{
				echo "                  <input class=\"transnb\" type=\"image\" src=\"images/save.gif\" value=\"Apply\" title=\"Save Discount\" DISABLED>\n";
			}
			else
...
function calcbyacc($estdata,$filters)
{
	...
	if (strlen($estdata) >=6)
	{
		...
		if (is_array($estAarray))
		{
			...
			foreach($estAarray as $n1=>$v1)
			{
				...
				$qry0 = "SELECT item,atrib1,atrib2,atrib3,qtype,catid,bullet,commtype FROM [".$MAS."acc] WHERE officeid='".$_SESSION['officeid']."' AND id='".$v1array[0]."';";
				$res0 = mssql_query($qry0);
				$row0 = mssql_fetch_array($res0);
				...
				if ($row0['qtype']!=32)
				{
					...
					if (isset($row0['bullet']) and $row0['bullet'] > 0)
					{
						$bullet="<img src=\"images/bullet_green.png\" title=\"SmartFeature (".$row0['bullet'].")\">";
					}
					else
					{
						$bullet='';
					...
					if ($row0['commtype']==0) {
						...
						if (isset($viewarray['com_base_rate']) && $viewarray['com_base_rate']!=0 && $row0['qtype']!=33)
						{
							$comm=$itemfromdb[0] * $viewarray['com_base_rate'];
						}
						else
						{
							$comm=$itemfromdb[1];
					...
					echo "              <td class=\"wh\" valign=\"bottom\" align=\"right\" width=\"60\">".$fdata_price."</td>\n";

					if ($comm!=0)
					{
						echo "              <td class=\"wh\" valign=\"bottom\" align=\"right\" width=\"60\">".$fcomm."</td>\n";
					}
					else
					{
	...
	{
	echo "           <tr>\n";
	echo "              <td colspan=\"6\" class=\"wh\" align=\"center\"><b>No Accessories Selected</b></td>\n";
			...
			foreach($estAarray as $n1=>$v1)
			{
				...
				*/
				...
				if ($row0['qtype']!=32)
				{
					...
					echo "           <tr>\n";
					echo "              <td class=\"wh\" valign=\"bottom\" align=\"left\">".$row1['name']."</td>\n";
					echo "              <td class=\"wh\" valign=\"bottom\" align=\"left\">\n";
					...
					echo "              </td>\n";
					echo "              <td class=\"wh\" align=\"center\" width=\"30\">".$itemfromdb[2]."</td>\n";
					echo "              <td class=\"wh\" align=\"center\" width=\"30\">".$itemfromdb[4]."</td>\n";
					echo "              <td class=\"wh\" align=\"right\">".$fdata_price."</td>\n";

					if ($comm!=0)
					{
						echo "              <td class=\"wh\" align=\"right\">".$fcomm."</td>\n";
					}
					else
					{
						echo "              <td class=\"wh\" align=\"right\"><img src=\"images/pixel.gif\"></td>\n";
...
function calcbyphsL($data,$bdata,$fdata,$job)
{
	...
	$qryA = "SELECT phsid,phscode,phsname,seqnum,phsname FROM phasebase WHERE phstype!='M' AND costing=1 ORDER BY seqnum ASC;";
	$resA = mssql_query($qryA);
	...
	$qryD = "SELECT secid,amt,label FROM CommissionSchedule WHERE oid=".$_SESSION['officeid']." AND jobid='".$viewarray['jobid']."' and jadd=0 and cbtype=4;";
	$resD = mssql_query($qryD);
	$rowD = mssql_fetch_array($resD);
	$nrowD= mssql_num_rows($resD);
	...
	$qryDa = "SELECT sum(amt) as amt FROM CommissionSchedule WHERE oid=".$_SESSION['officeid']." AND jobid='".$viewarray['jobid']."' and cbtype!=4;";
	$resDa = mssql_query($qryDa);
	$rowDa = mssql_fetch_array($resDa);
	$nrowDa= mssql_num_rows($resDa);
	
	if (isset($viewarray['sidm']) && $viewarray['sidm']!=0)
	{
		$qryE = "SELECT securityid,fname,lname FROM security WHERE securityid=".$viewarray['sidm'].";";
		$resE = mssql_query($qryE);
		$rowE = mssql_fetch_array($resE);
		
		$smc_text=$rowE['fname'].' '.$rowE['lname'];
	}
	else
	{
		$smc_text='';
	...
	if ($_REQUEST['action']!='est' and $viewarray['sidm']!=0 and $nrowD > 0 && isset($viewarray['cdate']) && $viewarray['cdate'] >= $viewarray['ncommdate'])
	{
		$phs504trig=1;
		$phs504Lamt=number_format($rowD['amt'], 2, '.', '');
		$phs504Ldes='Sales Manager Commission: '.$smc_text;
	}
	elseif ($_REQUEST['action']=='est' and $viewarray['sidm']!=0 and isset($viewarray['cdate']) && $viewarray['cdate'] >= $viewarray['ncommdate'])
	{
		//echo 'EST HIT<br>';
		$phs504trig=1;
		
		$qryEa = "SELECT rwdrate FROM CommissionBuilder WHERE oid=".$_SESSION['officeid']." and ctgry=4 and secid=".$viewarray['sidm']." and active=1;";
		...
		if ($nrowEa > 0)
		{			
			$smcperc=$rowEa['rwdrate'];
		}
		else
		{
			$qryEb = "SELECT rwdrate FROM CommissionBuilder WHERE oid=".$_SESSION['officeid']." and ctgry=4 and secid=0 and active=1;";
			$resEb = mssql_query($qryEb);
			$rowEb = mssql_fetch_array($resEb);
			$nrowEb =mssql_num_rows($resEb);
			
			if ($nrowEb > 0)
			{
				$smcperc=$rowEb['rwdrate'];
			}
			else
			{
				$smcperc=1;
		...
		if (isset($viewarray['ou']) && $viewarray['ou'] > 0)
		{
			$phs504Lamt=number_format(($viewarray['tcontract'] * $smcperc), 2, '.', '');
		}
		else
		{
			$phs504Lamt=number_format(($viewarray['tcontract'] * ($smcperc/2)), 2, '.', '');
		}
		
		if ($nrowEa!=0 or $nrowEb!=0)
		{
			$phs504Ldes='Sales Manager Commission: '.$smc_text.' @ '. ($smcperc * 100) . '%';
		}
		else
		{
			$phs504trig=0;
	...
	{
		echo 'TRG: '.$phs504trig.'<br>';
		echo 'AMT: '.$phs504Lamt.'<br>';
		echo 'DES: '.$phs504Ldes.'<br>';
	}
	
	if ($_SESSION['action']=='est' && isset($viewarray['cdate']) && $viewarray['cdate'] >= $viewarray['ncommdate'])
	{
		$tcomm=$viewarray['tcomm'];
	}
	elseif ($nrowDa > 0 && isset($viewarray['cdate']) && $viewarray['cdate'] >= $viewarray['ncommdate'])
	{
		$tcomm=number_format($rowDa['amt'], 2, '.', '');
	}
	else
	{
		$tcomm=$viewarray['tcomm'];
	...
	echo "           <tr>\n";
	echo "              <td class=\"wh\" align=\"center\" width=\"50\"><b>Code</b></td>\n";
	echo "              <td class=\"wh\" align=\"left\" width=\"100\"><b>Phase</b></td>\n";

	if (empty($_POST['showtotals']) || $_POST['showtotals']==0)
	{
		echo "              <td class=\"wh\" align=\"left\"><b>Labor Items</b></td>\n";
		echo "              <td class=\"wh\" align=\"center\" width=\"30\"><b>Quant</b></td>\n";
		echo "              <td class=\"wh\" align=\"center\" width=\"70\">";
	...
	{
		echo "              <td colspan=\"3\" class=\"wh\" align=\"left\"><b>Labor Items</b></td>\n";
	}
	
	//echo "              <td class=\"wh\" align=\"center\" width=\"100\"><b>Total</b></td>\n";
	echo "              <td class=\"wh\" align=\"center\" width=\"70\">";
	...
	{
		echo "              <td class=\"wh\" align=\"center\"><b>Adjust</b></td>\n";
		echo "              <td class=\"wh\" align=\"center\"><b>Adj Total</b></td>\n";
	}
	...
	while($rowA = mssql_fetch_row($resA))
	{
		...
		{
			if (isset($_REQUEST['TotalEstComm']) && !empty($_REQUEST['TotalEstComm']))
			{
				//$fcomm	=number_format($tcomm, 2, '.', '');
				$fcomm	=number_format($_REQUEST['TotalEstComm'], 2, '.', '');
			}
			else
			{
				$fcomm	=number_format($tcomm, 2, '.', '');
			...
			echo "           <tr class=\"objExportable\">\n";
			echo "              <td align=\"center\" class=\"wh\"><b>".$rowA[1]."</b></td>\n";
			echo "              <td align=\"left\" class=\"wh\"><b>".$rowA[2]."</b></td>\n";

			if (empty($_POST['showtotals'])||$_POST['showtotals']==0)
			{
				echo "              <td align=\"right\" class=\"wh\"></td>\n";
				echo "              <td align=\"right\" class=\"wh\"></td>\n";
				echo "              <td align=\"right\" class=\"wh\" width=\"70\"></td>\n";
			}
			else
			{
				echo "              <td colspan=\"3\" align=\"right\" class=\"wh\"></td>\n";
			}

			echo "              <td align=\"right\" class=\"wh\"><b>".$fcomm."</b></td>\n";
			...
			echo "           </tr>\n";
			$bctotal=$bctotal+$fcomm;
			$rctotal=$rctotal;
		}
		elseif ($rowA[1]=="504L" && $_SESSION['call']!='view_wo' && $phs504trig==1)
		{
			$tcomm	=$phs504Lamt;
			$fcomm	=number_format($tcomm, 2, '.', '');
			echo "           <tr class=\"objExportable\">\n";
			...
			if (empty($_POST['showtotals'])||$_POST['showtotals']==0)
			{
				echo "              <td align=\"left\" class=\"wh\">".$phs504Ldes."</td>\n";
				echo "              <td align=\"right\" class=\"wh\"></td>\n";
				echo "              <td align=\"right\" class=\"wh\" width=\"70\"></td>\n";
			}
			else
			{
				echo "              <td colspan=\"3\" align=\"left\" class=\"wh\">".$phs504Ldes."</td>\n";
			}

			echo "              <td align=\"right\" class=\"wh\"><b>".$phs504Lamt."</b></td>\n";
		...
		elseif ($rowA[1]=="505L" && $_SESSION['call']!='view_wo')
		{
			...
			echo "           <tr class=\"objExportable\">\n";
			echo "              <td align=\"center\" class=\"".$tbg."\"><b>".$rowA[1]."</b></td>\n";
			echo "              <td align=\"left\" class=\"".$tbg."\"><b>".$rowA[2]."</b></td>\n";

			if (empty($_POST['showtotals'])||$_POST['showtotals']==0)
			{
				echo "              <td align=\"right\" class=\"".$tbg."\"></td>\n";
				echo "              <td align=\"right\" class=\"".$tbg."\"></td>\n";
				echo "              <td align=\"right\" class=\"".$tbg."\" width=\"70\"></td>\n";
			}
			else
			{
				echo "              <td colspan=\"3\" align=\"right\" class=\"".$tbg."\"></td>\n";
			}

			echo "              <td align=\"right\" class=\"".$tbg."\"><b>".$froyalty."</b></td>\n";
		...
		{
			echo "           <tr>\n";
			echo "              <td class=\"wh\" align=\"left\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
	...
	if ($_SESSION['action']!="est" && !empty($viewarray['jobid'])||!empty($viewarray['njobid']))
	{
		...
		if ($nrowV > 0)
		{
			...
			{
				if ($rowV['raddncm_man']!=0)
				{
					//$pstcm_ar[$rowV['jadd']]=array('sid'=>$rowV['raddncm_man'],'raddcm_man'=>$rowV['raddncm_man']);
					$pstcm_ar[$rowV['jadd']]=$rowV['raddncm_man'];
					//echo $rowV['raddncm_man'];
				}
				...
				{
					echo "           <tr>\n";
					echo "              <td class=\"gray\" align=\"center\">60".$rowV['jadd']."L</td>\n";
					echo "              <td align=\"left\" class=\"gray\">Comm (503L)</td>\n";
					echo "              <td align=\"left\" class=\"gray\">Addendum Commission</td>\n";
					echo "              <td align=\"right\" class=\"gray\">n/a</td>\n";
					echo "              <td align=\"right\" class=\"gray\">". number_format($rowV['raddncm_man'], 2, '.', '') ."</td>\n";
					echo "              <td width=\"65\" class=\"gray\" align=\"right\"></td>\n";
					echo "           </tr>\n";
					$xaddpr=$rowV['raddncm_man'];
				}
				*/
				...
				if ($rowB['stax']==1)
				{
					...
					if (empty($_POST['showtotals']))
					{
						echo "           <tr>\n";
						echo "              <td class=\"gray\" align=\"center\">60".$rowV['jadd']."L</td>\n";
						echo "              <td align=\"left\" class=\"gray\">Misc (530L)</td>\n";
						echo "              <td align=\"left\" class=\"gray\">Sales Tax</td>\n";
						echo "              <td align=\"center\" class=\"gray\">".$fctaxrate."</td>\n";
						echo "              <td align=\"right\" class=\"gray\">".$ftaxamt."</td>\n";
						echo "              <td width=\"65\" class=\"gray\" align=\"right\"></td>\n";
						echo "           </tr>\n";
					}
				}
				...
				//echo		"3: ".$xaddpr,"<br>";
				echo "           <tr>\n";
				echo "              <td class=\"wh\" align=\"center\"><b>60".$rowV['jadd']."L</b></td>\n";
				echo "              <td align=\"left\" class=\"wh\"><b>".$add_type." Addendum<b></td>\n";
				echo "              <td colspan=\"3\" align=\"left\" class=\"wh\">".$shw_pmas_add."</td>\n";
				echo "              <td width=\"70\" class=\"wh\" align=\"right\"><b>";
...
function calcbyphsM($data,$fdata,$job)
{
	...
	if (empty($_POST['showtotals'])||$_POST['showtotals']==0)
	{
		echo "              <td class=\"wh\" align=\"left\"><b>Material Items</b></td>\n";
		echo "              <td class=\"wh\" align=\"center\" width=\"30\"><b>Quant</b></td>\n";
		echo "              <td class=\"wh\" align=\"center\" width=\"70\">";
	...
	{
		echo "              <td colspan=\"3\" class=\"wh\" align=\"left\"><b>Material Items</b></td>\n";
	}
	...
	while($rowA = mssql_fetch_row($resA))
	{
		...
		{
			echo "           <tr>\n";
			echo "              <td align=\"right\" class=\"wh\"><b>$rowA[4] Total</b></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
			echo "              <td align=\"right\" class=\"wh\"></td>\n";
...
function form_element_calc_ACC($id,$quan,$code,$amt,$ctype,$crate,$mquan,$chgproc)
{
	...
	//echo "Status: ".$viewarray['status']."<br>";
