...
if ($_SESSION['securityid']==2699999999999999999999999999999999)
{
	echo __FILE__.'<br>';
}
...
function BaseMatrix()
{
	...
	elseif (isset($_SESSION['call']) && $_SESSION['call']=="folder_maintenance")
	{
		folder_maintenance();
	}
	elseif (isset($_SESSION['call']) && $_SESSION['call']=="file_maintenance")
	{
		file_maintenance();
	}
	...
	if (isset($_FILES) && count($_FILES['userfile']) > 0)
	{
		foreach ($_FILES['userfile']['error'] as $n => $v)
		{
			...
			elseif ($v!=4)
			{
				...
				$terr=test_file($v);
				$err_cnt++;
				$err_txt=$err_txt."File: ".$_FILES['userfile']['name'][$n]."<br>Error: ".$terr[1]."<br>Result: No Action Taken <br>(Upload: ".$max_upload.")<br>(Post: ".$max_post.")<br>(". __LINE__ .")<p />";
			}
		}
...
function storefile_FS($fileName,$tempName,$fileSize,$fileType,$rcid,$roid,$fscid,$ruid)
{
	...
	$qryA = "select docid from jest..jestFileStore where oid=".$roid." and fscid=".$fscid." and filename='".$fileName."' and uid='".$ruid."';";
	$resA = mssql_query($qryA);
	$nrowA= mssql_num_rows($resA);
...
function stubfileimages($f)
{
	...
	elseif ($f == 'application/pdf' or $f==='pdf')
	{
...
function change_cat_CID() {
	...
	if ((isset($_REQUEST['docid']) && $_REQUEST['docid']!=0) && (isset($_REQUEST['fscid']) && $_REQUEST['fscid']!=0)) {
		...
		$qry0 = "SELECT docid FROM jest..jestFileStore WHERE docid=".(int) $_REQUEST['docid'].";";
		$res0 = mssql_query($qry0);
		$nrow0= mssql_num_rows($res0);
		
		if ($nrow0 > 0) {
			$qry1 = "UPDATE jest..jestFileStore SET fscid=".(int) $_REQUEST['fscid'].",udate=getdate() WHERE docid=".(int) $_REQUEST['docid'].";";
			$res1 = mssql_query($qry1);
		}
	}
...
function select_file_CID($ftype,$fscat_ar)
{
	if (isset($_REQUEST['cid']) && $_REQUEST['cid'] != 0)
	{
		...
		echo "<script type=\"text/javascript\" src=\"js/jquery_file_func.js\"></script>\n";
		echo "<div class=\"outerrnd noPrint\" style=\"width:750px\">\n";
		...
		echo "							<input type=\"hidden\" name=\"MAX_FILE_SIZE\" value=\"".$max_cstore."\" />\n";
		echo "							<table>\n";
		echo "								<tr>\n";
...
function parse_tree_live_OFF($t_ar,$fsaccess,$addfldr)
{
	ini_set('display_errors','On');
    error_reporting(E_ALL);
	...
	echo "</div>\n";
	echo "<div class=\"folder\">HOME</div>";
	
	if ($addfldr and $fsaccess >= 5)
	{
		echo "<div class=\"FileEditControl\">\n";
		echo "	<a href=\"#\"><img class=\"FolderAdd\" src=\"images/folder_add.png\" id=\"0\" height=\"14px\" width=\"14px\" title=\"Click to add a Folder\"></a>\n";
		echo "</div>\n";
	}
	...
	foreach ($t_ar as $n => $v) // 1st Tier Kick-off
	{
		if (is_array($v))
		{
			...
			{
				echo "<div class=\"FileEditControl\">\n";
				echo "	<a href=\"#\"><img class=\"FolderAdd\" src=\"images/folder_add.png\" id=\"".$n."\" height=\"14px\" width=\"14px\" title=\"Click to add a Folder\"></a>\n";
				
				if ($fsaccess >= 6)
				{
					echo "	<a href=\"#\"><img class=\"FolderDelete\" src=\"images/bin.png\" id=\"".$n."\" height=\"14px\" width=\"14px\" title=\"Click to Delete this Folder\"></a>\n";
				}
				
				echo "	<a href=\"#\"><img class=\"FileAdd\" src=\"images/folder_go.png\" id=\"".$n."\" height=\"14px\" width=\"14px\" title=\"Click to add a File to this Folder\"></a>\n";
				echo "</div>\n";
			}
			...
			if (is_array($v[1]))
			{
				parse_tree_node_OFF($v[1],$fsaccess,$addfldr);
			}
			...
			if (is_array($v[2]) and count($v[2]) > 0)
			{
				foreach ($v[2] as $fn => $fv)
				{
					...
					{
						echo "<div class=\"FileEditControl\">\n";
						echo "	<a href=\"#\"><img class=\"FileDelete\" src=\"images/bin.png\" id=\"".$fn."\" height=\"14px\" width=\"14px\" title=\"Click to Delete this File\"></a>\n";
						echo "</div>\n";
					}
...
function parse_tree_node_OFF($t_ar,$fsaccess,$addfldr)
{
	foreach ($t_ar as $n => $v)
	{
		if (is_array($v))
...
function build_tree_array_OFF($oid,$sl,$fc,$act,$incfiles)
{
	ini_set('display_errors','On');
    error_reporting(E_ALL);
	...
	if ($nrow0 > 0) // Build Folder Array
	{
		while ($row0 = mssql_fetch_array($res0))
		{
			...
			else
			{
				...
				$fc_ar[$row0['fscid']][0]=array($row0['fscatname'],$row0['comment']);
				$fc_ar[$row0['fscid']][1]=build_tree_subarray_OFF($row0['fscid'],$oid,$act,$incfiles);
...
function build_tree_subarray_OFF($pid,$oid,$act,$incfiles)
{
	$out =array();
	...
	if ($nrow0 > 0)
	{
		while ($row0 = mssql_fetch_array($res0))
		{
			...
			$out[$row0['fscid']][0]=array($row0['fscatname'],$row0['comment']);
			$out[$row0['fscid']][1]=build_tree_subarray_OFF($row0['fscid'],$oid,$act,$incfiles);
...
function list_file_OFF_NEW()
{
	echo "<script type=\"text/javascript\" src=\"js/jquery_file_func_loader.js\"></script>\n";
	echo "<div id=\"OfficeFileCabinet\"></div>\n";
}
...
function list_file_OFF()
{
	...
	if ((isset($rowp0['fsoffice']) and $rowp0['fsoffice']==1) or $row0['officeid']==89)
	{
		...
		{
			//$tree_ar=build_tree_array_OFF($_SESSION['officeid'],$row0['filestoreaccess'],0,1,true);
			...
			{
				echo "		<td align=\"left\"><b>".$_SESSION['offname']." File Cabinet</b></td>\n";
				echo "		<td align=\"right\"><b>ATTENTION:</b> All active files in the Corporate File Share are viewable by Blue Haven Office Admins</td>\n";
			}
			else
			{
				echo "		<td align=\"left\"><b>".$_SESSION['offname']." File Cabinet</b></td>\n";
				echo "		<td  align=\"right\">Access Level ".$row0['filestoreaccess'].$fstar[$row0['filestoreaccess']]."</td>\n";
			}
			...
			echo "<input type=\"hidden\" id=\"FFOID\" value=\"".$_SESSION['officeid']."\">";
			echo "<input type=\"hidden\" id=\"FFSID\" value=\"".$_SESSION['securityid']."\">";
			...
			echo "				<div id=\"FTree\">\n";
			echo "					<p>\n";
			echo "						<div id=\"FileStoreTreeResultHTML\"></div>\n";			
			echo "					</p>\n";
			echo "				</div>\n";
			...
			if ($row0['filestoreaccess'] >= 5)
			{
				echo "				<div id=\"FList\">\n";
				echo "					<p>\n";
				echo "						<div id=\"FileStoreListResultHTML\"></div>\n";
				echo "					</p>\n";
				echo "				</div>\n";
			}
...
function list_file_OFFNEW()
{
	...
	{		
		if ($row0['filestoreaccess'] > 5)
		{
			...
			echo "<script type=\"text/javascript\" src=\"js/jquery_file_func_OFF.js\"></script>\n";
			...
			echo "<input id=\"FFOID\" type=\"hidden\" value=\"".$_SESSION['officeid']."\">\n";
			echo "<table class=\"transnb\" width=\"950px\">\n";
			echo "	<tr>\n";
			echo "		<td align=\"left\">\n";
			echo "			<div id=\"filelisting_OFF_2\">\n";
			echo "				<ul>\n";
			echo "					<li><a href=\"#FTree\">Office Files</a></li>\n";
			echo "					<li><a href=\"#FList\">Office File Search</a></li>\n";
			echo "					<li><a href=\"#SSpace\">Storage Space</a></li>\n";
			echo "				</ul>\n";
			echo "				<div id=\"FTree\">\n";
			echo "					<div id=\"FileStoreTreeJSON\"></div>";
			echo "				</div>\n";
			echo "				<div id=\"FList\">\n";
			echo "					<p>\n";
			...
			echo "							<tr>\n";
			echo "								<td align=\"left\">\n";
			echo "									Search <input id=\"FFNameSrch\" type=\"text\" value=\"Enter Full or Partial Filename...\" onFocus=\"ClearField('FFNameSrch');\" size=\"40\">\n";
			echo "								</td>\n";
			echo "							</tr>\n";
			echo "						</table>\n";
			echo "						<div id=\"FileStoreSearchResult\"></div>";
			echo "					</p>\n";
			echo "				</div>\n";
			echo "				<div id=\"SSpace\">\n";
			echo "					<div id=\"FileStoreCapacityResult\"></div>";
			echo "				</div>\n";
...
function list_file_ENT()
{
	...
	{		
		if ($row0['filestoreaccess'] >= 5)
		{		
			echo "<script type=\"text/javascript\" src=\"js/jquery_file_func_ENT.js\"></script>\n";
			//echo "<table class=\"outer\" width=\"950px\">\n";
			//echo "	<tr>\n";
			...
			echo "<input id=\"FFOID\" type=\"hidden\" value=\"197\">\n";
			echo "<table class=\"transnb\" width=\"950px\">\n";
			echo "	<tr>\n";
			echo "		<td align=\"left\">\n";
			echo "			<div id=\"filelisting_OFF_2\">\n";
			echo "				<ul>\n";
			echo "					<li><a href=\"#FTree\">Shared Files</a> <img class=\"JMStooltip\" src=\"images/help.png\" title=\"Expand branches of the Tree below by clicking on the triangle icon next to the Folder. Click on File Icons to download the corresponding file.\"></li>\n";
			echo "					<li><a href=\"#FList\">Shared Files Search</a> <img class=\"JMStooltip\" src=\"images/help.png\" title=\"Enter the Full or Partial File name in the Search Field below.\"></li>\n";
			echo "				</ul>\n";
			echo "				<div id=\"FTree\">\n";
			echo "					<p>\n";
			echo "						<div id=\"FileStoreTreeJSON\"></div>";
			...
			echo "							</tr>\n";
			echo "						</table>\n";
			echo "						<div id=\"FileStoreSearchPanel\"></div>";
			echo "					</p>\n";
			echo "				</div>\n";
			echo "			</div>\n";
			echo "		</td>\n";
...
function getCustomerEmailFiles($cid,$oid) {
	$out=array();
	$fs	="F:\\FileStore\\CustomerEmailFiles\\".$cid;
	...
	if (is_dir($fs)) {
		...
		foreach ($ff as $n=>$v) {
			if ($v!='.' and $v!='..') {
				$ft=(preg_match('/pdf/i',$v))?'application/pdf':'';
				$ft=(preg_match('/jpg/i',$v))?'image/jpeg':'';
				$out[]=array(
					'docid'=>0,
					'filename'=>$v,
					'filetype'=>$ft,
					'filesize'=>filesize($fs.'\\'.$v),
					'filestore'=>$fs,
					'adate'=>filectime($fs.'\\'.$v),
					'udate'=>filectime($fs.'\\'.$v),
					'cid'=>$cid,
					'oid'=>$oid,
					'active'=>1,
					'fscid'=>0,
...
function list_file_CID() {
	$files=array();
	$cid=(isset($_REQUEST['cid']) && $_REQUEST['cid'] != 0)?$_REQUEST['cid']:0;
	
	if ($_SESSION['tester']==99) {
		echo '<pre>';
		print_r($_REQUEST);
		echo '</pre>';
	}
	...
	if ((isset($rowp0['fscustomer']) and $rowp0['fscustomer']==1) or $row0['officeid']==89)
	{
		if (isset($row0['filestoreaccess']) and $row0['filestoreaccess'] >= 1)
		{
			if (isset($_REQUEST['cid']) && $_REQUEST['cid'] != 0)
			{
				$files	=getCustomerEmailFiles($cid,$rowp0['officeid']);
				...
				while ($row=mssql_fetch_array($res)) {
					$files[$row['docid']]=array(
						'docid'=>$row['docid'],
						'filename'=>$row['filename'],
						'filetype'=>$row['filetype'],
						'filesize'=>$row['filesize'],
						'filestore'=>$row['filestore'],
						'adate'=>strtotime($row['adate']),
						'udate'=>strtotime($row['udate']),
						'oid'=>$row['oid'],
						'sid'=>$row['sid'],
						'cid'=>$row['cid'],
						'fscid'=>$row['fscid'],
				...
				echo "<table width=\"750px\">\n";
				echo "	<tr>\n";
				echo "		<td align=\"left\"><b>".$row1['cfname']." ".$row1['clname']." File Cabinet</b></td>\n";
				echo "		<td align=\"center\"><img src=\"images/pixel.gif\"></td>\n";
				echo "		<td align=\"right\">\n";
				echo "			<table>\n";
				echo "				<tr>\n";
				echo "					<td>Customer OneSheet</td>\n";
				echo "					<td>\n";
				echo "						<div class=\"noPrint\">\n";
				...
				{
					//echo "	<br />\n";
					echo "	<table class=\"outer\" width=\"750px\">\n";
					echo "		<thead>\n";
					echo "			<tr>\n";
					echo "				<td class=\"tblhd\"><img src=\"images/pixel.gif\"></td>\n";
					echo "				<td class=\"tblhd\"><img src=\"images/pixel.gif\"></td>\n";
					echo "				<td class=\"tblhd\" align=\"center\">Name</td>\n";
					echo "				<td class=\"tblhd\" align=\"center\">Category/Folder</td>\n";
					echo "				<td class=\"tblhd\" align=\"center\">Size</td>\n";
					echo "				<td class=\"tblhd\" align=\"center\">Uploaded</td>\n";
					echo "				<td class=\"tblhd\" align=\"center\">Updated</td>\n";
					echo "				<td class=\"tblhd\"><img src=\"images/pixel.gif\"></td>\n";
					echo "				<td class=\"tblhd\"><img src=\"images/pixel.gif\"></td>\n";
					echo "				<td class=\"tblhd\"><img src=\"images/pixel.gif\"></td>\n";
					echo "				<td class=\"tblhd\"><img src=\"images/pixel.gif\"></td>\n";
					...
					foreach ($files as $nf=>$vf) {
						...
						echo "				</td>\n";
						echo "				<td align=\"right\">".number_format($vf['filesize'])." kb</td>\n";
						echo "				<td align=\"center\">".date('m/d/Y G:i A',$vf['adate'])."</td>\n";
						echo "				<td align=\"center\">".date('m/d/Y G:i A',$vf['udate'])."</td>\n";
						echo "				<td align=\"center\" width=\"20px\">\n";
						
						if ($_SESSION['securityid']==26999999999999999999999999) {
							echo '<img id="docid_'.$vf['docid'].'" class="EmailFile pointer" src="images/email.png" title="Email this File">';
						}
						
						echo "				</td>\n";
						echo "				<td align=\"center\" width=\"20px\">\n";
						//echo $vf['active'].':'.$vf['docid'].':'.$row0['filestoreaccess'];
						if ($vf['active']==0 and $vf['docid']!=0 and $row0['filestoreaccess'] >= 6) {
							echo "					<form method=\"post\">\n";
							echo "					<input type=\"hidden\" name=\"action\" value=\"file\">\n";
						...
						if ($vf['docid']!=0) {
							...
							if ($row0['filestoreaccess'] >= 6)
							{
								if ($vf['active']==1)
								{
									...
									echo "					<input class=\"transnb\" type=\"image\" src=\"images/bin_empty.png\" onClick=\"return ConfirmDeactivateFile();\" title=\"Deactivate File\">\n";
								}
								else
								{
									echo "					<input type=\"hidden\" name=\"call\" value=\"undelete_file_CID\">\n";
									echo "					<input class=\"transnb\" type=\"image\" src=\"images/accept.png\" onClick=\"return ConfirmRestoreFile();\" title=\"Restore File\">\n";
								}
							}
						...
						if ($vf['active']==1 and $vf['docid']!=0)
						{
							...
							echo "					<form action=\"http://jms.bhnmi.com/export/fileout.php\" target=\"_new\" method=\"post\">\n";
							echo "					<input type=\"hidden\" name=\"cid\" value=\"".$vf['cid']."\">\n";
							echo "					<input type=\"hidden\" name=\"filename\" value=\"".$vf['filename']."\">\n";
							echo "					<input type=\"hidden\" name=\"storetype\" value=\"file_fc\">\n";
							echo "					<input class=\"transnb\" type=\"image\" src=\"images/download.gif\" alt=\"Download File\">\n";
							echo "					</form>\n";
						}
